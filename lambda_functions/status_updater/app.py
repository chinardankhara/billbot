...
To deploy this status_updater Lambda function, the following AWS 
infrastructure components must be configured:

1. API GATEWAY HTTP API ENDPOINT:
   - Create an HTTP API (not REST API) for better performance and cost
   - Configure a route: POST /webhooks/stripe
   - Set integration target to the status_updater Lambda function
   - Enable CORS if needed for debugging from browser tools
   - Example CloudFormation/CDK:
     ```
     ApiGateway:
       Type: AWS::ApiGatewayV2::Api
       Properties:
         Name: jetty-webhooks-api
         ProtocolType: HTTP
...
2. LAMBDA FUNCTION CONFIGURATION:
   ...
   - Environment Variables:
     ...
     * DYNAMODB_TABLE_NAME: Name of the invoices table
     * AWS_REGION: AWS region (usually auto-populated)
   
3. IAM PERMISSIONS:
   The Lambda execution role needs these permissions:
   - logs:CreateLogGroup, logs:CreateLogStream, logs:PutLogEvents
   - dynamodb:Query (on PaymentIntentIndex GSI)
   - dynamodb:UpdateItem (on main table)
   
4. DYNAMODB GSI:
   Add PaymentIntentIndex to the existing Invoices table:
   - Partition Key: payment_intent_id (String)
   - Projection: ALL (to access all attributes for updates)
   
5. STRIPE WEBHOOK CONFIGURATION:
   In Stripe Dashboard (https://dashboard.stripe.com/webhooks):
   - Add new webhook endpoint
   - URL: https://{api-gateway-id}.execute-api.{region}.amazonaws.com/webhooks/stripe
   - Events to send: payment_intent.succeeded
   - Copy the signing secret to STRIPE_WEBHOOK_SECRET environment variable

6. SECURITY CONSIDERATIONS:
   - Webhook signature verification is MANDATORY (implemented above)
   - Use HTTPS only (API Gateway enforces this)
   - Store webhook secret securely (consider AWS Secrets Manager for production)
   - Monitor webhook failures and implement retry logic if needed
   - Consider rate limiting at API Gateway level to prevent abuse
...
7. MONITORING AND DEBUGGING:
   ...
   - Stripe Dashboard: Monitor webhook delivery attempts and failures

8. DEPLOYMENT STEPS:
   1. Deploy Lambda function with status_updater code
   2. Create API Gateway HTTP API and route
   3. Add PaymentIntentIndex GSI to DynamoDB table
   4. Configure Stripe webhook with API Gateway URL
   5. Test with Stripe CLI: stripe listen --forward-to {api-gateway-url}
   
Example API Gateway URL format:
https://abc123def4.execute-api.us-east-1.amazonaws.com/webhooks/stripe

This URL should be registered in Stripe Dashboard webhooks section.
""" 